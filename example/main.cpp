...
int main() {
    ...
    for (int i = 0; i < positions.size(); i++) {
        ...
        string best_move = ai.search(board);
        auto end = chrono::high_resolution_clock::now();

        double ms = chrono::duration<double, milli>(end - start).count();
        times_ms.push_back(ms);

        cout << "[" << (i + 1) << "/" << positions.size() << "] "
             << positions[i].substr(0, 40) << "...  "
             << "move: " << best_move << "  "
             << "time: " << static_cast<int>(ms) << " ms" << endl;
    }

    const double total = accumulate(times_ms.begin(), times_ms.end(), 0.0);
    const double avg = total / times_ms.size();
    const double min_t = *ranges::min_element(times_ms);
    const double max_t = *ranges::max_element(times_ms);

    cout << "-----------------------------------" << endl;
    cout << "Total:   " << static_cast<int>(total) << " ms" << endl;
    cout << "Average: " << static_cast<int>(avg) << " ms" << endl;
    cout << "Min:     " << static_cast<int>(min_t) << " ms" << endl;
    cout << "Max:     " << static_cast<int>(max_t) << " ms" << endl;
