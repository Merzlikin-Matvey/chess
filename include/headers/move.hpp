...
namespace chess {
    ...
    // Bits 12-14: move type (0=normal, 1=w_l_castle, 2=w_s_castle, 3=b_l_castle, 4=b_s_castle, 5=en_passant,
    // 6=pawn_double) Bits 15-17: moving piece type (0-5) Bits 18-20: captured piece type (0-5, 7=none) Bit  21:
    // attacker color (0=white, 1=black) Bit  22:    captured color (0=white, 1=black) Bits 23-25: promotion piece type
    // (0-5, 7=none)
    ...
    public:
        ...
        Move(uint8_t from, uint8_t from_color, uint8_t from_type, uint8_t to, uint8_t to_color, uint8_t to_type,
             bool w_l_castling, bool w_s_castling, bool b_l_castling, bool b_s_castling, bool pawn_double_move,
             bool en_passant, uint8_t pawn_change_type);
        ...
        static Move make(const uint8_t from, const uint8_t to, const uint8_t piece_type, const uint8_t attacker_color,
                         const uint8_t captured_type, const uint8_t captured_color, const MoveType mt,
                         const uint8_t promotion = 7) {
            Move m;
            m.data = from & 0x3F | (to & 0x3F) << 6 | (mt & 0x7) << 12 | (piece_type & 0x7) << 15 |
                     (captured_type & 0x7) << 18 | (attacker_color & 0x1) << 21 | (captured_color & 0x1) << 22 |
                     (promotion & 0x7) << 23;
            return m;
        }
    };

    std::ostream& operator<<(std::ostream& ostream, Move move);
    ...
    public:
        ...
        void clear();
        void resize(const int new_size) { _size = new_size; }

        std::array<Move, 218>::iterator begin() { return moves.begin(); }

        std::array<Move, 218>::iterator end() { return moves.begin() + _size; }

        std::array<Move, 218>::const_iterator begin() const { return moves.begin(); }

        std::array<Move, 218>::const_iterator end() const { return moves.begin() + _size; }
