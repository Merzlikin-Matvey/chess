...
static int count_pawn_mask(Bitboard mask, const Bitboard friendly, const uint8_t color) {
    mask &= ~friendly;
    const Bitboard promo_rank = (color == chess::White)
        ? bitboard_operations::rows[7]
        : bitboard_operations::rows[0];
...
int chess::Board::count_legal_moves() {
    ...
    if (is_check()) {
        ...
        Bitboard up_right_pinned_pawns = piece_bitboards[color][Pawn] & up_right_pin_mask;
        Bitboard up_left_pinned_pawns = piece_bitboards[color][Pawn] & up_left_pin_mask;
        count += count_pawn_mask(masks::get_pawn_mask(vertical_pinned_pawns, *this) & check_mask & vertical_pin_mask, friendly, color);
        count += count_pawn_mask(masks::get_pawn_long_mask(vertical_pinned_pawns, *this) & check_mask & vertical_pin_mask, friendly, color);
        if (color == White) {
            count += count_pawn_mask(masks::get_pawn_right_mask(up_right_pinned_pawns, *this) & check_mask & up_right_pin_mask, friendly, color);
            count += count_pawn_mask(masks::get_pawn_left_mask(up_left_pinned_pawns, *this) & check_mask & up_left_pinned_pawns, friendly, color);
        } else {
            count += count_pawn_mask(masks::get_pawn_right_mask(up_right_pinned_pawns, *this) & check_mask & up_right_pin_mask, friendly, color);
            count += count_pawn_mask(masks::get_pawn_left_mask(up_left_pinned_pawns, *this) & check_mask & up_left_pinned_pawns, friendly, color);
        }
        ...
        while (pinned_rooks) {
            square = bitboard_operations::bitScanForward(pinned_rooks);
            bitboard_operations::set_0(pinned_rooks, square);
            count += count_mask(masks::get_rook_mask(*this, square) & (horizontal_pin_mask | vertical_pin_mask) & check_mask, friendly);
        }
        ...
        while (pinned_bishops) {
            square = bitboard_operations::bitScanForward(pinned_bishops);
            bitboard_operations::set_0(pinned_bishops, square);
            count += count_mask(masks::get_bishop_mask(*this, square) & (up_right_pin_mask | up_left_pin_mask) & check_mask, friendly);
        }
        ...
        while (pinned_rooks) {
            square = bitboard_operations::bitScanForward(pinned_rooks);
            bitboard_operations::set_0(pinned_rooks, square);
            count += count_mask(masks::get_rook_mask(*this, square) & (horizontal_pin_mask | vertical_pin_mask), friendly);
        }
        ...
        while (pinned_bishops) {
            square = bitboard_operations::bitScanForward(pinned_bishops);
            bitboard_operations::set_0(pinned_bishops, square);
            count += count_mask(masks::get_bishop_mask(*this, square) & (up_right_pin_mask | up_left_pin_mask), friendly);
        }
        ...
        // Castling
        {
            if (color == White) {
                if (w_l_castling &&
                    bitboard_operations::get_bit(piece_bitboards[White][Rook], 0) &&
                    !bitboard_operations::get_bit(all, 1) &&
                    !bitboard_operations::get_bit(all, 2) &&
                    !bitboard_operations::get_bit(all, 3) &&
                    bitboard_operations::get_bit(piece_bitboards[White][King], 4) &&
                    !is_position_attacked(2) && !is_position_attacked(3))
                    count++;
                if (w_s_castling &&
                    bitboard_operations::get_bit(piece_bitboards[White][Rook], 7) &&
                    !bitboard_operations::get_bit(all, 5) &&
                    !bitboard_operations::get_bit(all, 6) &&
                    bitboard_operations::get_bit(piece_bitboards[White][King], 4) &&
                    !is_position_attacked(5) && !is_position_attacked(6))
                    count++;
            } else {
                if (b_l_castling &&
                    bitboard_operations::get_bit(piece_bitboards[Black][Rook], 56) &&
                    !bitboard_operations::get_bit(all, 57) &&
                    !bitboard_operations::get_bit(all, 58) &&
                    !bitboard_operations::get_bit(all, 59) &&
                    bitboard_operations::get_bit(piece_bitboards[Black][King], 60) &&
                    !is_position_attacked(58) && !is_position_attacked(59))
                    count++;
                if (b_s_castling &&
                    bitboard_operations::get_bit(piece_bitboards[Black][Rook], 63) &&
                    !bitboard_operations::get_bit(all, 61) &&
                    !bitboard_operations::get_bit(all, 62) &&
                    bitboard_operations::get_bit(piece_bitboards[Black][King], 60) &&
                    !is_position_attacked(61) && !is_position_attacked(62))
                    count++;
            }
        }
    }
