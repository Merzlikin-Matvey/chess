...
void chess::Board::castling(chess::MoveArray* moves) {
    ...
    if (color == White) {
        // Long
        if (bitboard_operations::get_bit(piece_bitboards[White][Rook], 0) and !bitboard_operations::get_bit(all, 1) and
            !bitboard_operations::get_bit(all, 2) and !bitboard_operations::get_bit(all, 3) and
        ...
        // Short
        if (bitboard_operations::get_bit(piece_bitboards[White][Rook], 7) and !bitboard_operations::get_bit(all, 5) and
            !bitboard_operations::get_bit(all, 6) and bitboard_operations::get_bit(piece_bitboards[White][King], 4)) {
...
chess::MoveArray& chess::Board::get_legal_moves() {
    ...
    if (is_check()) {
        ...
        pawn_mask_to_moves(masks::get_pawn_long_mask(vertical_pinned_pawns, *this) & check_mask & vertical_pin_mask, 16,
                           &legal_moves, false);
        if (color == White) {
            pawn_mask_to_moves(masks::get_pawn_right_mask(up_right_pinned_pawns, *this) & check_mask &
                                   up_right_pin_mask,
                               9, &legal_moves, false);
            pawn_mask_to_moves(masks::get_pawn_left_mask(up_left_pinned_pawns, *this) & check_mask &
                                   up_left_pinned_pawns,
                               7, &legal_moves, false);
        } else {
            pawn_mask_to_moves(masks::get_pawn_right_mask(up_right_pinned_pawns, *this) & check_mask &
                                   up_right_pin_mask,
    ...
    while (pinned_rooks) {
        square = bitboard_operations::bitScanForward(pinned_rooks);
        bitboard_operations::set_0(pinned_rooks, square);
        mask_to_moves(masks::get_rook_mask(*this, square) & (horizontal_pin_mask | vertical_pin_mask), square, color,
                      Rook, &legal_moves);
    }
